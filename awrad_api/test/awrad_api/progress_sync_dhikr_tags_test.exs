defmodule AwradApi.ProgressSyncDhikrTagsTest do
  use AwradApi.DataCase, async: false

  import AwradApi.AccountsFixtures

  alias AwradApi.ProgressSync
  alias AwradApi.ProgressSync.{Document, EntityRecord, Transfer}
  alias AwradApi.Repo
  ...
  test "capability-gated tag rows do not crowd non-tag entities out of transfer limits",
       context do
    previous = Application.get_env(:awrad_api, :progress_sync_max_transfer_records)
    Application.put_env(:awrad_api, :progress_sync_max_transfer_records, 1)

    on_exit(fn ->
      if is_nil(previous) do
        Application.delete_env(:awrad_api, :progress_sync_max_transfer_records)
      else
        Application.put_env(:awrad_api, :progress_sync_max_transfer_records, previous)
      end
    end)
