...
- `AwradApi.Accounts.Token` — JWT access tokens + refresh token logic
- `AwradApiWeb.Plugs.ApiAuth` — Bearer token plug for API pipeline
- `AwradApiWeb.Api.AuthController` — JSON auth endpoints
- `AwradApiWeb.FallbackController` — JSON error handling
