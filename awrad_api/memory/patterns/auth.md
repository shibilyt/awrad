...
Awrad API serves two clients with different auth mechanisms from a single `User` schema:

| Client | Mechanism | Module | Lifetime |
|---|---|---|---|
| Web (LiveView) | Session + signed cookie | `AwradApiWeb.UserAuth` | 14 days |
| Mobile API | JWT access + rotating DB refresh | `AwradApiWeb.Plugs.ApiAuth` + `AwradApi.Accounts.Token` | 15 min / 180 days |

## Web Auth (Session)

Powered by `phx.gen.auth`. Session tokens are stored in the `users_tokens` table with `context = "session"`.

### Plugs (from `AwradApiWeb.UserAuth`)
...
Module: `AwradApi.Accounts.Token` (uses `Joken.Config`)
...
Signing secret: per-environment config at `config :awrad_api, AwradApi.Accounts.Token, signing_secret: "..."`
...
Module: `AwradApiWeb.Plugs.ApiAuth`
...
- `lib/awrad_api/accounts.ex` — Accounts context (public API)
- `lib/awrad_api/accounts/user.ex` — User schema with email/password changesets
- `lib/awrad_api/accounts/user_token.ex` — DB token schema (session, magic link, change email)
- `lib/awrad_api/accounts/token.ex` — JWT access + DB refresh token logic
- `lib/awrad_api/accounts/scope.ex` — Scope struct (wraps current user)
- `lib/awrad_api_web/user_auth.ex` — Session auth plugs (generated)
- `lib/awrad_api_web/plugs/api_auth.ex` — JWT Bearer auth plug
- `lib/awrad_api_web/controllers/api/auth_controller.ex` — JSON auth endpoints
