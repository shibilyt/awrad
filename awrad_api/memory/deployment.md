...
1. `test` — `mix precommit` against a Postgres 16 service container. Gates
   everything below.
2. `build` — Buildx builds `awrad_api/Dockerfile` for `linux/amd64` and pushes
...
`/app/bin/migrate` runs `AwradApi.Release.migrate` — `Ecto.Migrator` without
...
`AwradApi.Mailer` still uses `Swoosh.Adapters.Local` in production, so
