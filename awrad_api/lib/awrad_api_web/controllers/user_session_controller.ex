defmodule AwradApiWeb.UserSessionController do
  ...
  def create(conn, %{"user" => %{"email" => email, "password" => password} = user_params}) do
    with :ok <- BrowserAuthProtection.allow?(conn, "password_login", email, 30, 10, 900) do
  ...
  def create(conn, %{"user" => %{"email" => email}}) do
    with :ok <- BrowserAuthProtection.allow?(conn, "magic_link", email, 5, 5, 3600) do
