...
#
#     PHX_SERVER=true bin/awrad_server start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :awrad_server, AwradServerWeb.Endpoint, server: true
end
...
if value = System.get_env("PROGRESS_SYNC_ENABLED") do
  config :awrad_server, :progress_sync_enabled, value in ~w(true 1)
end

if value = System.get_env("PROGRESS_SYNC_MAINTENANCE_ENABLED") do
  config :awrad_server, :progress_sync_maintenance_enabled, value in ~w(true 1)
end
...
if config_env() == :prod do
  ...
  config :awrad_server, AwradServer.Repo,
    # ssl: true,
    url: database_url,
  ...
  config :awrad_server, AwradServer.Accounts.AuthRateLimiter, pepper: rate_limit_pepper

  config :awrad_server, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
