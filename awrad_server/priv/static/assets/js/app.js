...
`
  ...
  var calculationMethods = {
    karachi: () => CalculationMethod_default.Karachi(),
    north_america: () => CalculationMethod_default.NorthAmerica(),
    mwl: () => CalculationMethod_default.MuslimWorldLeague(),
    egypt: () => CalculationMethod_default.Egyptian(),
    umm_al_qura: () => CalculationMethod_default.UmmAlQura(),
    moon_sighting: () => CalculationMethod_default.MoonsightingCommittee(),
    dubai: () => CalculationMethod_default.Dubai(),
    kuwait: () => CalculationMethod_default.Kuwait(),
    qatar: () => CalculationMethod_default.Qatar(),
    singapore: () => CalculationMethod_default.Singapore()
  };
  var homePrayerDefinitions = [
    ["fajr", "Fajr"],
    ["dhuhr", "Dhuhr"],
    ["asr", "Asr"],
    ["maghrib", "Maghrib"],
  ...
  function resolvePracticeDay({
    ...
    try {
      ...
      const effectiveDate = now.getTime() >= maghrib.getTime() ? addCalendarDays(civilDate, 1) : civilDate;
      return {
        civilDate,
        effectiveDate,
  ...
  function resolvePrayerTimes({
    ...
    try {
      ...
      const rows = homePrayerDefinitions.map(([key, name]) => ({ key, name, at: prayerTimes[key] })).filter(({ at }) => at instanceof Date && !Number.isNaN(at.getTime()));
  ...
  function browserCivilDate(now, timeZone) {
  ...
  function addCalendarDays(civilDate, days) {
    const date = /* @__PURE__ */ new Date(`${civilDate}T12:00:00Z`);
  ...
  function formatCountdown(now, target) {
    const totalMinutes = Math.max(Math.floor((target.getTime() - now.getTime()) / (1e3 * 60)), 0);
  ...
  function locationFingerprint(dataset) {
    return [
  ...
  function withLocationName(location2, locationName) {
    const normalizedName = String(locationName || "").trim();
    return normalizedName ? { ...location2, location_name: normalizedName } : { ...location2 };
  }
  ...
  Hooks2.BrowserClock = {
    mounted() {
      ...
      this.timer = window.setInterval(this.pushBrowserContext, 6e4);
    ...
    requestBrowserLocation: function() {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          const message = error.code === error.PERMISSION_DENIED ? "Location permission was not granted." : "We could not read this device's location.";
          this.setLocationStatus(message);
          this.finishLocationRequest();
        },
        { enableHighAccuracy: false, maximumAge: 3e5, timeout: 1e4 }
      );
    },
    ...
    reverseBrowserLocation(location2) {
      return new Promise((resolve) => {
        let settled = false;
        ...
        window.setTimeout(() => finish(null), 6e3);
      });
    },
    ...
    locationFingerprint() {
      return locationFingerprint(this.el.dataset);
    },
