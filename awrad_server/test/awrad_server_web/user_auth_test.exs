defmodule AwradServerWeb.UserAuthTest do
  ...
  @remember_me_cookie "_awrad_api_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AwradServerWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "keeps the browser installation across logout", %{conn: conn} do
      installation_id = Ecto.UUID.generate()

      conn =
        conn
        |> init_test_session(%{web_installation_id: installation_id})
        |> UserAuth.log_out_user()
  ...
  describe "live view authentication hooks" do
    test "mounts the current scope and browser installation from the LiveView session", %{
      user: user
    } do
      user_token = Accounts.generate_user_session_token(user)
      ...
      assert {:cont, socket} =
               UserAuth.on_mount(
                 :mount_current_scope,
                 %{},
                 %{"user_token" => user_token, "web_installation_id" => "browser-id"},
                 socket
               )

      assert socket.assigns.current_scope.user.id == user.id
      assert socket.assigns.web_installation_id == "browser-id"
    end

    test "halts unverified practice sessions at the verification page" do
      user = unconfirmed_user_fixture()
      user_token = Accounts.generate_user_session_token(user)
      socket = %Phoenix.LiveView.Socket{}

      {:cont, socket} =
        UserAuth.on_mount(:mount_current_scope, %{}, %{"user_token" => user_token}, socket)

      assert {:halt, socket} = UserAuth.on_mount(:require_verified_user, %{}, %{}, socket)
      assert {:redirect, %{to: "/users/verification-required"}} = socket.redirected
    end
  end
end
