defmodule AwradServerWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "keeps the browser installation when the session is renewed", %{
      conn: conn,
      user: user
    } do
      ...
      conn =
        ...
        |> post(~p"/users/log-in", %{
          "user" => %{"token" => elem(generate_user_magic_link_token(user), 0)}
        })
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_awrad_api_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert AwradServer.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
