...
LiveView pages live in `lib/awrad_server_web/live/` and use the `:app` layout:
...
The `use AwradServerWeb, :live_view` macro provides:
- `Phoenix.LiveView` with `layout: {AwradServerWeb.Layouts, :app}`
...
```elixir
# lib/awrad_server_web/components/layouts.ex
def app(assigns) do
...
1. Create module in `lib/awrad_server_web/live/my_page_live.ex`
...
Defined in `lib/awrad_server_web/components/core_components.ex`:
...
- `lib/awrad_server_web.ex` — macro definitions (`:live_view`, `:html`, etc.)
- `lib/awrad_server_web/components/layouts.ex` — layout definitions
- `lib/awrad_server_web/components/layouts/root.html.heex` — root HTML template
- `lib/awrad_server_web/components/core_components.ex` — shared components
- `lib/awrad_server_web/live/home_live.ex` — example LiveView page
