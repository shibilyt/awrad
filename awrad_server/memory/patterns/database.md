...
Database client is `AwradServer.Repo` configured in `lib/awrad_server/repo.ex`.
...
|---|---|---|
| `users` | `AwradServer.Accounts.User` | User accounts (email, hashed_password, confirmed_at) |
| `users_tokens` | `AwradServer.Accounts.UserToken` | Auth tokens (session, refresh, magic link, change email) |
...
- `AwradServer.Accounts` — user registration, login, password/email management, token lifecycle
...
def email_changeset(user, attrs, opts \\ []) do
  ...
  |> unsafe_validate_unique(:email, AwradServer.Repo)
...
- `lib/awrad_server/repo.ex` — Ecto Repo
- `lib/awrad_server/accounts/user.ex` — User schema + changesets
- `lib/awrad_server/accounts/user_token.ex` — Token schema + queries
