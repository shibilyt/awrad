defmodule AwradServer.ProgressSync do
  @moduledoc """
  ...
  @doc """
  Ensures that one server-managed browser installation has an active actor.

  The installation identifier is stable for a browser, while the actor remains
  scoped to the authenticated user. The registry advisory lock makes the
  first request from multiple tabs converge on one actor instead of creating
  parallel incarnations.
  """
  def ensure_actor(%Scope{user: %User{id: user_id}}, installation_id) do
    with {:ok, installation_id} <- uuid_v4(installation_id) do
      ...
      Repo.transaction(fn ->
        ensure_head!(user_id)
        lock_actor_registry!(user_id)

        active_actor =
          Repo.one(
            from actor in Actor,
              where:
                actor.user_id == ^user_id and actor.installation_id == ^installation_id and
                  is_nil(actor.retired_at) and actor.lease_expires_at > ^now,
              order_by: [desc: actor.incarnation],
              limit: 1,
              lock: "FOR UPDATE"
          )
        ...
        case active_actor do
          %Actor{} = actor ->
            actor
            |> Ecto.Changeset.change(last_seen_at: now, lease_expires_at: lease_expires_at)
            |> Repo.update!()
  ...
  def execute_count_command(%Scope{user: %User{id: user_id}} = scope, attrs)
    ...
    execute_command(scope, attrs, fn revision -> apply_count_command(user_id, attrs, revision) end)
  end

  @doc "Executes a count command using the actor's next sequence on the server."
  def execute_next_count_command(scope, actor_id, attrs),
    do: execute_next_count_command(scope, actor_id, attrs, fn -> :ok end)

  @doc "Executes a count command with an optional transaction-local preflight."
  def execute_next_count_command(
        %Scope{user: %User{id: user_id}},
        actor_id,
        attrs,
        preflight
      )
      when is_map(attrs) and is_function(preflight, 0) do
    with {:ok, command_id} <- uuid_v4(value(attrs, :command_id)),
         {:ok, actor_id} <- uuid_v4(actor_id),
         payload when is_map(payload) <- value(attrs, :payload),
         {:ok, payload_hash} <- CanonicalJSON.hash(payload) do
      Repo.transaction(fn ->
        ensure_head!(user_id)
        ...
        case lock_actor(user_id, actor_id) do
          ...
          %Actor{} = actor ->
            if actor_available?(actor) do
              result =
                case receipt_for_command(user_id, command_id) do
                  %CommandReceipt{} = receipt ->
                    sequence =
                      if receipt.actor_id == actor.id,
                        do: receipt.actor_sequence,
                        else: actor.next_expected_sequence

                    replay_or_adopt(receipt, actor, sequence, payload_hash)

                  nil ->
                    sequence = actor.next_expected_sequence
                    ...
                    apply_next_command(
                      ...
                      head,
                      fn revision ->
                        with :ok <- preflight.() do
                          apply_count_command(user_id, command_attrs, revision)
                        end
                      end
                    )
                end

              touch_actor_to_revision!(actor, result.revision)
              result
            else
              Repo.rollback(:actor_expired)
            end
        end
      end)
      |> normalize_transaction()
  ...
  @doc "Acknowledges a canonical browser read at the current user head."
  def acknowledge_actor_at_head(%Scope{user: %User{id: user_id}}, actor_id) do
    with {:ok, actor_id} <- uuid_v4(actor_id) do
      Repo.transaction(fn ->
        head = lock_head!(user_id)
        actor = lock_actor(user_id, actor_id)
        now = DateTime.utc_now(:second)
        ...
        cond do
          ...
          not is_nil(actor.retired_at) or DateTime.compare(actor.lease_expires_at, now) != :gt ->
            Repo.rollback(:actor_expired)

          true ->
            frontier = max(actor.applied_revision, head.revision)
            safe_frontier = max(actor.safe_compaction_revision, frontier)

            actor
            |> Ecto.Changeset.change(
              applied_revision: frontier,
              safe_compaction_revision: safe_frontier,
              last_seen_at: now,
              lease_expires_at: DateTime.add(now, @actor_lease_seconds, :second)
            )
            |> Repo.update!()
  ...
  defp lock_actor_registry!(user_id) do
    Repo.query!(
      "SELECT pg_advisory_xact_lock(hashtextextended($1, 0))",
      ["sync-actors:" <> to_string(user_id)]
    )
  ...
  defp touch_actor_to_revision!(actor, revision) do
    now = DateTime.utc_now(:second)

    actor
    |> Ecto.Changeset.change(
      applied_revision: max(actor.applied_revision, revision),
      safe_compaction_revision: max(actor.safe_compaction_revision, revision),
      last_seen_at: now,
      lease_expires_at: DateTime.add(now, @actor_lease_seconds, :second)
    )
    |> Repo.update!()
  ...
  defp actor_available?(%Actor{retired_at: nil, lease_expires_at: lease_expires_at}) do
    DateTime.compare(lease_expires_at, DateTime.utc_now(:second)) == :gt
  end

  defp actor_available?(_actor), do: false

  defp apply_count_command(user_id, attrs, revision) do
