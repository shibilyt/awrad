defmodule AwradServerWeb.UserAuth do
  use AwradServerWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    web_installation_id = get_session(conn, :web_installation_id)
    user_token && Accounts.delete_user_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      AwradServerWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    conn
    |> renew_session(nil)
    |> maybe_put_session(:web_installation_id, web_installation_id)
  ...
  @doc "Mounts the browser session scope and installation for LiveViews."
  def on_mount(:mount_current_scope, _params, session, socket) do
    scope =
      case session_value(session, :user_token) do
        nil -> nil
        token -> session_scope(token)
      end
    ...
    socket =
      ...
      |> Phoenix.Component.assign(
        :web_installation_id,
        session_value(session, :web_installation_id)
      )
  ...
  def on_mount(:require_verified_user, _params, _session, socket) do
    case socket.assigns.current_scope do
      %Scope{user: %{confirmed_at: %DateTime{}}} ->
        {:cont, socket}

      %Scope{user: %{}} ->
        {:halt, Phoenix.LiveView.redirect(socket, to: ~p"/users/verification-required")}
  ...
  defp authenticated_scope?(%Scope{user: %{}}), do: true
